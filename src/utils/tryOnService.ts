...
function generateTryOnPrompt(clothingInfo: ClothingInfo, fitType: FitType): string {
  ...
  if (isTop) {
    ...
    const mandatoryBase = `Mandatory: The person's chest, belly, and overall body shape are exactly as in Image 1 with zero modification. Garment length matches Image 2 exactly. Hem hanging freely over and outside the pants waistband. Ignore how the person wears their original clothing in Image 1 — the wearing style comes only from Image 2. All body parts in correct anatomical proportion to each other.`
    switch (fitType) {
      case 'tight':
        return `Virtual try-on. The person in Image 1 is wearing the ${itemType} from Image 2. ${style} Subject: The exact same person from Image 1 — identical face, body shape, body size, chest size, skin tone, hair, pose. ALL original clothing on the upper body is completely removed and replaced by the ${itemType} from Image 2. The ${itemType} matches the exact color, pattern, sleeve style, sleeve cut, sleeve length, and shirt length from Image 2. ${composition} Fit: Only the width is tighter — the garment width looks one size too small for this body. The fabric is stretched taut against the skin with 0mm air gap. The shape of the shoulders and hip bones is visible pressing through the material. Zero wrinkles, zero folds, zero bunching. Chest area lays flat and smooth. ${mandatoryBase} 100% garment length accuracy from Image 2. Prohibitions: Added chest volume, loose fabric, tucked-in shirt even if original clothing is tucked, original upper body clothing visible, jacket, hoodie, sweater, layering, cropped or shortened shirt, longer hemline than Image 2, shorter hemline than Image 2, change to body size or proportions, disproportionate body parts.`
      case 'comfortable':
        return `Virtual try-on. The person in Image 1 is wearing the ${itemType} from Image 2. ${style} Subject: The exact same person from Image 1 — identical face, body shape, body size, skin tone, hair, pose. ALL original clothing on the upper body is completely removed and replaced by the ${itemType} from Image 2. The ${itemType} matches the exact color, pattern, sleeve style, sleeve cut, sleeve length, and shirt length from Image 2. ${composition} Fit: Only the width is larger — the shirt is one full size wider than the person's body. 50mm of visible air gap between the fabric and the torso on each side. The shoulder seams drop 30mm past the natural shoulder bone. The sleeves are visibly wider than the arms. 5-7 prominent vertical folds run down the front. The body shape is hidden by excess material in the width. ${mandatoryBase} The shirt is wider, not longer. 100% garment length accuracy from Image 2. Prohibitions: Fitted look, fabric touching the torso sides, tucked-in shirt even if original clothing is tucked, original upper body clothing visible, jacket, hoodie, sweater, layering, longer hemline than Image 2, shorter hemline than Image 2, dress-like length, change to body size or proportions, disproportionate body parts.`
      default:
        return `Virtual try-on. The person in Image 1 is wearing the ${itemType} from Image 2. ${style} Subject: The exact same person from Image 1 — identical face, body shape, body size, skin tone, hair, pose. ALL original clothing on the upper body is completely removed and replaced by the ${itemType} from Image 2. The ${itemType} matches the exact color, pattern, sleeve style, sleeve cut, sleeve length, and shirt length from Image 2. ${composition} Fit: Standard retail fit. The fabric skims the body with approximately 10mm of space between skin and fabric. Shoulder seams sit exactly on the shoulder bone. A few natural creases at the waist. Body shape suggested but not defined through the fabric. ${mandatoryBase} 100% garment length accuracy from Image 2. Prohibitions: Skin-tight fabric, oversized look, tucked-in shirt even if original clothing is tucked, original upper body clothing visible, jacket, hoodie, sweater, layering, longer hemline than Image 2, shorter hemline than Image 2, change to body size or proportions, disproportionate body parts.`
    }
  } else {
