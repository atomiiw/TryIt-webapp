...
function generateTryOnPrompt(clothingInfo: ClothingInfo, fitType: FitType, gender: string = 'unknown'): string {
  ...
  if (isTop && isMale) {
    ...
    const base = `GENERATE A REALISTIC VIRTUAL TRY-ON IMAGE. You will receive 2 images: 1) Person/avatar, 2) Top clothing item (${itemName}). Show the person from the first image wearing the top from the second image. CRITICAL: You MUST generate and return a new image, not just text. IMPORTANT: COMPLETELY REMOVE and DISCARD ALL existing upper body clothing — every shirt, jacket, hoodie, undershirt, long sleeve, short sleeve, EVERYTHING on the upper body and arms is GONE. The person's upper body and arms are completely bare first. Then PUT ON ONLY the ${itemColor} ${itemType} from the second image. The ONLY fabric visible on the upper body and arms is the ${itemName} from Image 2. Where the ${itemName} does not cover the arms, BARE SKIN is visible — no other fabric, no other sleeve, no undershirt. Do not layer over existing clothes. The new garment should fit naturally on the person's upper body, maintaining realistic proportions and shadows. The shirt hangs freely over the pants, not tucked in. PRESERVE EXACTLY: the person's face, facial expression, body shape, body size, chest size, belly size, body pose, body position, background, lighting, camera angle from the original avatar image. Zero body modification — no added chest volume, no added belly, no altered body shape. Color: pixel-accurate to Image 2 — exact hue, saturation, brightness.`
