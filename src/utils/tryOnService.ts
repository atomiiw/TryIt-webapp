    ...
    switch (fitType) {
      case 'tight': {
        const fitDesc = isStructured
          ? `The ${itemName} is one size too small. The zipper or buttons strain to close — visible pulling and gapping at the chest. Sleeves end above the wrist, too short. The torso section is visibly too narrow, sides pull tight. The silhouette is noticeably smaller than the person's frame.`
        ...
        return `${base} FIT: TIGHT. ${fitDesc} The garment length remains exactly as shown in Image 2 — tightness affects width only, not length. ${quality} Prohibitions: loose fabric, added chest/breast volume, added belly volume, altered body shape, original clothing visible, layering, tucked-in shirt, original sleeves visible, long sleeves when Image 2 shows short sleeves, color shift from Image 2, cropped shirt, shorter hemline than Image 2.`
      }
      case 'comfortable': {
      ...
      default: { // regular
        ...
        return `${base} FIT: REGULAR. ${regularFit} The garment length remains exactly as shown in Image 2. If Image 2 shows long sleeves, the person wears long sleeves. Do NOT keep the original sleeve length. ${quality} Prohibitions: skin-tight fabric, oversized look, added chest/breast volume, added belly volume, altered body shape, ANY trace of original clothing, layering, collar/sleeve/hem visible underneath, tucked-in shirt, original sleeves visible, long sleeves when Image 2 shows short sleeves, color shift from Image 2, shorter hemline than Image 2, longer hemline than Image 2.`
      }
    }
  } else if (isTop) {
