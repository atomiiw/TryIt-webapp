  ...
  if (isTop && isMale) {
    ...
    switch (fitType) {
      ...
      case 'comfortable': {
        ...
        return `${base} FIT: COMFORTABLE — OBVIOUSLY OVERSIZED, WIDER NOT LONGER. ${comfortFit} The garment length remains exactly as shown in Image 2. The shirt is dramatically wider, not longer. ${quality} Keep the person the same size in frame — crop the garment hem at the frame edge rather than shrinking the person. Prohibitions: fitted fabric, fabric touching torso sides, fabric conforming to body shape, shrinking person, added chest/breast volume, added belly volume, altered body shape, layering, tucked-in shirt, color shift from Image 2, dress-like length, shorter hemline than Image 2, longer hemline than Image 2, original clothing visible on torso or collar, original sleeves visible on arms, any fabric on arms beyond the ${itemName}'s own sleeves. Most critical: the oversized fit is immediately obvious, and bare skin shows on arms beyond the garment's sleeve line.`
      }
      default: { // regular
