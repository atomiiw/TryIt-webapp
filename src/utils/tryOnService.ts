...
function generateTryOnPrompt(clothingInfo: ClothingInfo, fitType: FitType): string {
  ...
  if (isTop) {
    ...
    const mandatory = `Mandatory: Garment length matches Image 2 exactly. Hem hanging freely over and outside the pants waistband. Ignore how the person wears their original clothing in Image 1 — the wearing style comes only from Image 2. All body parts in correct anatomical proportion to each other. 100% garment length accuracy from Image 2.`
    switch (fitType) {
      case 'tight':
